...
@section('body')
    ...
    <div class="casa-mess">
        <div class="tag-list">
            <img src="{{ config('config.photo_folder').$casa->attachment->filepath }}" alt="{{ $casa->name }}"  width="100%" >
            <p>标签：
    ...
    <div class="casa-mess" id="others">
        <section>
            ...
            @foreach($casas as $casa)
                <div class="card">
                    <a href="/casa/{{ $casa->id }}" >
                        <img src="{{ config('config.photo_folder').$casa->attachment->filepath }}" width="100%" alt="{{ $casa->name }}">
                        <h3>{{ $casa->name }}</h3>
                        <p>标签：
